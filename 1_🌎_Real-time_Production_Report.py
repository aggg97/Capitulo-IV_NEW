...
if has_frac_data:
    ...
    # Merge con datos de fractura para cantidad_fracturas
    frac_dedup = df_frac_main.drop_duplicates("sigla")[["sigla", "cantidad_fracturas"]]
    df_eff = (
        peaks_main
        .merge(frac_dedup, on="sigla", how="inner")
        .query("formprod == 'VMUT' and sub_tipo == 'SHALE'")
        .query("fluido == 'Petrolífero' and Qo_peak > 0 and cantidad_fracturas > 0")
        .copy()
    )
    ...
    if not df_eff.empty:
        eff_agg = (
            ...
            .agg(
                p50  ="median",
                p10  =lambda x: x.quantile(0.9),
                p90  =lambda x: x.quantile(0.1),
                count="count",
            )
            .reset_index()
            .dropna(subset=["p50"])
        ...
        fig_eff.add_trace(go.Scatter(
            ...
            mode="lines+markers+text", name="P50",
            line=dict(color=ROLL_COLOR_OIL, width=3), marker=dict(size=8),
        ...
        fig_eff.update_layout(
            title="Eficiencia de Capital: Qo Pico / Etapa por Campaña — Petrolífero VMUT Shale",
            xaxis_title="Campaña de Perforación",
            yaxis_title="Qo Pico / Etapa (m³/d/etapa)",
            template="plotly_white", legend=LEGEND_BOTTOM,
            annotations=[dict(
                ...
                bordercolor=BROWNFIELD_COLOR, borderwidth=1,
            )],
        )
        st.plotly_chart(fig_eff, use_container_width=True)

# 7d. Infographic brownfield
st.info(
    """
